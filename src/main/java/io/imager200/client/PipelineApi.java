...
public class PipelineApi {
   ...
   * 
   * @param name the name of the pipeline (required)
   * @param body image binary data, acceptable formats: jpeg,png,bmp,gif (depending on operations) (required)
   * @return SuccessResponse
   * @throws ApiException if fails to make API call
   */
  public SuccessResponse pipelinePost(String name, File body) throws ApiException {
    ApiResponse<SuccessResponse> localVarResponse = pipelinePostWithHttpInfo(name, body);
    return localVarResponse.getData();
  }
   ...
   * @throws ApiException if fails to make API call
   */
  public ApiResponse<SuccessResponse> pipelinePostWithHttpInfo(String name, File body) throws ApiException {
    HttpRequest.Builder localVarRequestBuilder = pipelinePostRequestBuilder(name, body);
    try {
      HttpResponse<InputStream> localVarResponse = memberVarHttpClient.send(
          localVarRequestBuilder.build(),
          HttpResponse.BodyHandlers.ofInputStream());
  ...
  private HttpRequest.Builder pipelinePostRequestBuilder(String name, File body) throws ApiException {
    // verify the required parameter 'name' is set
    if (name == null) {
    ...
    if (body == null) {
      throw new ApiException(400, "Missing the required parameter 'body' when calling pipelinePost");
    }
